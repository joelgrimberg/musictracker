...
function CreatePlaylistDialog({ open, onOpenChange }: { open: boolean, onOpenChange: (open: boolean) => void }) {
    const playlistNameRef = useRef<HTMLInputElement>(null);
    const { mutate: createPlaylist, isLoading } = client.createPlaylist.useMutation({
        onSuccess: () => {
            onOpenChange(false);
