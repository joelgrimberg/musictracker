...
export async function getMetadataForUrl(mediaUrl: string, source: TrackSources) {
    try {
        ...
        if (e instanceof z.ZodError) {
            throw Error(`Unexpected oEmbed API response from noembed API: ${e.issues[0].message} for data: ${e.issues[0].path[0]} `)
        }
        throw e;
    }
}