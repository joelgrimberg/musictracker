...
export const SongList: FunctionComponent = () => {
  ...
  return (
    <div className="grid grid-cols-2 gap-4 rounded-xl sm:grid-cols-4 md:gap-8 bg-white/10 p-4 text-white">

      {songs && songs.length? '': 'quickly add your first song so I can show it here 🥳' }
      {songs && songs.map(({ title, uuid, coverUrl }) => (
        ...
        <div
          ...
          <div className="grid grid-cols-2 h-10 w-400">
            <div className="w-300">
              <h2>{title}</h2>
            </div>
            ...
            <div className='flex justify-end'>
              <form onSubmit={onSubmit} key={uuid} className="flex flex-col gap-4">
                ...
                <button disabled={isLoading} onClick={removeTrack(uuid)} type="submit">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 hidden fill-slate-400 hover:fill-red-600 group-hover:inline-block">
